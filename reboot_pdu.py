...
def measure_boot_time(ip, log_dir, video_source, pdu):
    ...
    with open(result_file, 'a') as f:
        f.write("KPI," + str(expected_kpi) + "\n")  # Ligne initiale avec valeur par défaut
    ...
    subprocess.run(["snmpset", "-v1", "-c", "public", f"{pdu}", "i", "3"])
    time.sleep(5)
...
def main(config, log_dir):
    try:
        ...
        measure_boot_time(ip, log_dir, video_source, pdu)
    except Exception as e:
